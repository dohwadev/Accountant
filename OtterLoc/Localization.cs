...
public static class LocalizationDict<T> where T : Enum
{
    ...
    public static bool RegisterName(T key, ClientLanguage lang, string en, string jp, string de, string fr, string ko)
        => lang switch
        {
            ClientLanguage.Japanese => Register(key, jp),
            ClientLanguage.German   => Register(key, de),
            ClientLanguage.French   => Register(key, fr),
            ClientLanguage.Korean   => Register(key, ko),
            _                       => Register(key, en),
        };

    public static bool RegisterName(T key, ClientLanguage lang, SeString se, int payloadEn, int payloadJp, int payloadDe, int payloadFr, int payloadKo)
        => lang switch
        {
            ...
            ClientLanguage.Korean   => Register(key, (se.Payloads[payloadKo] as TextPayload)?.Text?.Trim() ?? string.Empty),
